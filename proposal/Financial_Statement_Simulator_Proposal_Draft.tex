...
Statement extractor & \textbf{\textcolor{gold}{Net-new}} & Reads a filing (tagged data or PDF) into a structured statement in the source standard, using redundant cross-checking tools; carries the near-zero read-error target. \\ \hline
...
\lead{navytwo}{Objective.} Build two things: a robust extractor that reads a filing into a structured statement, and the per-standard encode/decode adapters with the common state space, so that extraction is accurate and the encode/decode round trip is lossless. The extraction uses LLM and traditional methods that cross-check each other. Sequence US GAAP first, then add IFRS as an extension on the same scaffolding.
\lead{navytwo}{Extraction (filing to structured statement), the robust part.}
\begin{itemize}
  \item Document paths, the core capability: a deterministic table parser on the HTML or PDF structure, and regular-expression and rule checks on the linearized text, each consuming a different representation of the filing; an LLM with deterministic decoding and schema-constrained output performs the semantic mapping of native line items onto the statement schema. These paths alone must meet a measured bar (the PDF-only mode of Section~\ref{sec:robustness}), since untagged documents are what generalize beyond public filings.
...
\lead{gold}{Acceptance.} Two gates. Extraction meets the per-field accuracy target on the test set for both standards, including a PDF-only ablation in which the tag path is withheld (measured at its own bar), with seeded read-errors caught and disagreements surfaced rather than silently resolved. The encode/decode pair achieves perfect reconstruction: the round trip through the state space reproduces each structured statement exactly under its own standard.
...
\begin{enumerate}
  ...
  \item Which mode gates the Part I acceptance test: tagged public filings, or an untagged PDF (a private firm, another jurisdiction) at the same bar? The pipeline is built for both and the PDF-only mode is measured separately; the question is which bar the MVP must clear.
\end{enumerate}
